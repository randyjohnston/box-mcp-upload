...
- Keep browser-to-Box uploads as the default. Only folder-restricted `base_upload` tokens reach the browser.
...
- Replace local staging with private object storage and an upload worker. For browser-blocked Box traffic, send small chunks to the application origin, reassemble in storage, and queue the MCP-backed Box transfer. Return a job ID and poll progress; retries must be idempotent.
...
Authentication mode/app variables set the initial UI default. Only provision credentials for flows the deployment should expose; users can explicitly connect any configured app.

Set server-only variables separately for Preview and Production; never prefix secrets with `NEXT_PUBLIC_`.

| Deployment                   | Variables                                                                                                                                     |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| OAuth, MCP preferred         | `BOX_DEFAULT_APP=mcp`, `BOX_MCP_CLIENT_ID`, `BOX_MCP_CLIENT_SECRET`                                                      |
| OAuth, platform preferred    | `BOX_DEFAULT_APP=platform`, `BOX_OAUTH_CLIENT_ID`, `BOX_OAUTH_CLIENT_SECRET`                                             |
| Optional alternate OAuth app | Configure the other credential pair; reconnection is explicit and remains per-user.                                                           |
| CCG                          | `BOX_DEFAULT_APP=ccg`, `BOX_CCG_CLIENT_ID`, `BOX_CCG_CLIENT_SECRET`, `BOX_CCG_SUBJECT_TYPE`, corresponding subject ID, `BOX_CCG_ROOT_FOLDER_ID` |
